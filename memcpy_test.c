...
static void try_malloc(size_t bytes)
{
    ...
    size_t total_bytes = sizeof(struct test_struct)+(bytes-1);  // size test_struct needs to be in order
                                                                ...
                                                                // for <bytes> bytes
    if (bytes < ARRAYBYTES)
    {   printf("try to convince SQ that I really DO know how to bounds-check...\n"); return; }
    ...
    // Now try to copy ARRAYBYTES into p1->open_ended_array
                                                                   // <<-- SQ will ALWAYS report this line
    memcpy(&(p1->open_ended_array[0]), static_array, ARRAYBYTES);  // <<-- as a BUG:
                                                                   // <<--    Memory copy function overflows
                                                                   // <<--    the destination buffer
}

void main()
{
    // this allocates 10x as much space as we need, just to show this is NOT some "off-by-1" style problem
